...
podman run --rm -ti -e PODMAN_VERSION=${PODMAN_VERSION} -v ${SRC_DIR}/${BUILD_OS}/${PRODUCT}:/tmp docker.io/${BUILD_OS_NAME}/leap:${BUILD_OS_VERSION_ID} /tmp/build_in_container.sh

# EXTRACT RPMS
OUT_DIR="${ARTIFACTS_DIR}/packages/${BUILD_OS}"
mkdir -p ${OUT_DIR}/SRPMS
mv ${SRC_DIR}/${BUILD_OS}/${PRODUCT}/podman/rpm/SRPMS/*.rpm ${OUT_DIR}/SRPMS/
mkdir -p ${OUT_DIR}/RPMS/${ARCH}
mv ${SRC_DIR}/${BUILD_OS}/${PRODUCT}/podman/rpm/RPMS/${ARCH}/*.rpm ${OUT_DIR}/RPMS/${ARCH}/
mkdir -p ${OUT_DIR}/RPMS/noarch
mv ${SRC_DIR}/${BUILD_OS}/${PRODUCT}/podman/rpm/RPMS/noarch/*.rpm ${OUT_DIR}/RPMS/noarch/

# SAVE binary for userspace
OUT_DIR="${ARTIFACTS_DIR}/${SARUS_SUITE_DIR}/bin"
mkdir -p ${OUT_DIR}
cp ${SRC_DIR}/${BUILD_OS}/${PRODUCT}/podman/rpm/BUILD/podman-*/bin/podman ${OUT_DIR}/
