...
PARALLAX_URL="${PARALLAX_BASE_URL}/v${PARALLAX_VERSION}/parallax-v${PARALLAX_VERSION}-${OS}-${ARCH}"
...
cd ${TMP_DIR}/download
curl -sO ${PARALLAX_URL}
curl -sO ${PARALLAX_MOUNT_PROGRAM_URL}

# INSTALL
mkdir -p ${ARTIFACTS_DIR}/${SARUS_SUITE_DIR}
cd ${TMP_DIR}/download
mv parallax-v${PARALLAX_VERSION}-${OS}-${ARCH} ${ARTIFACTS_DIR}/${SARUS_SUITE_DIR}/parallax
